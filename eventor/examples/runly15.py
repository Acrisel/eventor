...
def construct_and_run():     
    ...
    if config.startswith('/private'):
        config = config[8:]   
    ev = evr.Eventor(name=os.path.basename(__file__), logging_level=logging.INFO, config=config, store=db, shared_db=True, import_module=["examples.example_00_prog", "examples.runly15_types", ],)
