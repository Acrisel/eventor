...
from acrilib import NamedSingleton
from acrilib import Threaded
import threading
...
class ResourcePool(NamedSingleton):
    ...
    def __repr__(self):
        return "ResourcePool( class: {}, policy: {})".format(self.__resource_cls.__name__, self.__policy)
    ...
    def _get(self, sync=False, count=1, wait=-1, callback=None, hold_time=None, ticket=None):
        ...
        if resource_limit > -1 and count > resource_limit:
            raise ResourcePoolError("Trying to get count ({}) larger than resource limit ({})"
                                    .format(count, resource_limit))
    ...
    def put(self, *resources):
        ...
        for ticket, (condition, count, caller) in list(self.__awaiting.items()):
            logger.debug("{}, {} found {} awaiting; require: {}, available: {}."
                         .format(caller, self.name, ticket, count, len(self.__available_resources)))
...
class Requestors(object):
    ...
    def reserve(self, request, wait=-1, callback=None, hold_time=None, expire=None,):
        ...
        self.__requests[request_id] = \
            self.Request(request_id=request_id, request=request, wait=wait, callback=callback, hold_time=hold_time, expire=expire)
